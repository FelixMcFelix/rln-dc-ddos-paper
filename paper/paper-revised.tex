...
We find that the existing work for this task \cite{DBLP:journals/eaai/MalialisK15} fails to account for congestion-aware traffic (i.e., TCP) and deployment environments with high host density per egress point, achieving poor results due to an overly coarse view of the network.
...
These are designed to make effective decisions irrespective of protocol, and to act on individual flows at the edge of network topologies with arbitrary shape and size.

We offer an in-depth investigation into suitable features for an automatic DDoS mitigation system, with qualitative and quantitative justification (\cref{sec:rethinking-the-state-space}).
These features have been suggested by past studies into DDoS prevention, and have been independently tested within their own contexts.
...
\section{Modelling Normal Traffic}\label{sec:a-new-normal}
...
To paint a more current picture, we compare our work against an effective modern-day algorithmic approach, SPIFFY \cite{DBLP:conf/ndss/KangGS16}.
SPIFFY tests a proportion of flows by rerouting them through an alternate path with higher bandwidth, and observing how their speed has changed some time later.
...
Interestingly, SPIFFY is unable to effectively protect constant bitrate legitimate traffic.
...
SPIFFY remains ineffective at protecting this class of traffic.
...
Here, SPIFFY greatly outperforms all of the RL-based methods.
...
SPIFFY continues to outperform the RL-based methods.
...
What we offer above many of the alternate approaches we discuss in \cref{sec:related-work} are potentially more flexible deployments and low-overhead decision-making, without requiring active measurement or the network resources and capabilities that the most effective techniques rely upon.
