% !TeX document-id = {b3181d9a-ffe0-4331-a65c-c7fb8f275531}
% !BIB program = biber
...
%\colorlet{shadecolor}{uofgcobalt!20}
...
These features have been suggested by past studies into DDoS prevention, and have been independently tested in their own contexts.
Our study is the first attempt to quantify the individual efficacy of each in an RL-focussed setting.
...
Observing that past work has been compared only against AIMD \cite{DBLP:journals/ton/YauLLY05}, we additionally compare our work against SPIFFY \cite{DBLP:conf/ndss/KangGS16}, reuniting two divergent strands of research and grounding the study of RL-based DDoS defences.
\cbend
...
An attacker's goal is to minimise the fair-share bandwidth allocation that a server can give to a host which connects to it, and they are expected to act rationally in its pursuit.
...
\cbstart Agents are co-located with each egress switch (i.e., $k$ ingress points from other ASes require $k$ agents), all employ the same action model or agent design, and control the proportion of upstream packets from each external host to discard.
...
Only the ingress/egress nodes from a network need to be OpenFlow-enabled, because it is advantageous to perform filtering as close to a flow's source as possible.
...
The main difference is that a traditional network needs this additional hardware, and does not allow for an operator to dynamically determine where the "edge" of their network lies through vNF relocation.
\cbend
...
\cbstart In the event that the path from an agent to its destination is shorter than 4 hops, we duplicate (in order of preference) the load measurement of a middle hop or the last hop. \cbend
...
\cbstart Rather than having the network push the data to an agent, the agent requests this information about active flows periodically to isolate it from non-control-plane traffic and to eliminate the risk of resource exhaustion by excessive requests. \cbend
...
Naturally, this implies that an agent must carry work forward (and coalesce state updates) when the \emph{host density} $n>3$ (\cref{sec:evaluation}); this behaviour is not explicitly a property of network size.
...
\begin{figure}
	\centering
	\includegraphics[width=0.65\linewidth]{../plots/ftprep-tcp-cap-box}
	\vspace{-0.75cm}
	\caption{
...
\begin{figure}
	\centering
	\vspace{-0.75cm}
	\includegraphics[width=0.65\linewidth]{../plots/ftprep-tcp-laf-cap-box}
	\vspace{-0.75cm}
	\caption{
...
The main element required to move to a per-source model is a feature set which offers high predictive power, so that behavioural differences are apparent to an agent.
...
\cbstart
Marl is already known to outperform the AIMD \cite{DBLP:journals/ton/YauLLY05} approach to DDoS mitigation, although the state of the art has long since moved on.
To paint a more current picture, we compare our work against an effective modern-day approach, SPIFFY \cite{DBLP:conf/ndss/KangGS16}.
SPIFFY tests a proportion of flows by routing them through an alternate path with higher bandwidth, and observing how their speed has changed some time later.
We compare ourselves against this approach to position our new RL agent designs against the modern state of the art, observing that SPIFFY has a similar mode of interaction to RL-based systems (taking action, observing an effect, and acting once again) and does not rely on protocol characteristics or signatures.
We make the simplifying assumption that a suitable unused path exists (with identical bandwidth to the server's link).
We test \SI{10}{\percent} of active flows at a time (according to the authors' observation that there is a factor of \num{10} difference between the ideal and achieved bandwidth expansion factor), excluding flows below \SI{50}{\kilo\bit\per\second} and requiring a \SI{3}{$\times$} bandwidth expansion from legitimate flows, making a judgement after \SI{5}{\second}.
\cbend
...
Host allocations at the beginning of each episode were generated pseudorandomly to ensure fairness between episodes---a host is malicious with probability $\operatorname{P}(\mathit{malicious})$, and is benign otherwise.
...
This is a slight simplification of \citeauthor{DBLP:journals/eaai/MalialisK15}'s \textquote{online} experiment \cite{DBLP:journals/eaai/MalialisK15}, choosing fewer teams but remaining as a single server with a fan-out network.
...
\cbstart For our choices of $n$ in both topologies, we observe $N_{\mathit{hosts}} \in \left\{24, 48, 96, 192\right\}$, and an expected number of malicious hosts $\mathbb{E}\left[N_{\mathit{attackers}}\right] \in \left\{9.6, 19.2, 38.4, 76.8\right\}$.
...
We present the average rewards for all combinations of these factors in \crefrange{tab:av-vals}{tab:av-ecmp-vals}---these provide a rough idea of expected performance, giving the highest-performing model in bold and underlining the best RL-based model.
...
\begin{table}
	...
	\cbstart
	\resizebox{0.8\linewidth}{!}{
		\expandableinput ../tables/tnsm-tree-avg-reward-spiffy.tex
	}
	\cbend
\vspace{-0.25cm}
\end{table}
\begin{table}
	...
	\cbstart
	\resizebox{0.8\linewidth}{!}{
		\expandableinput ../tables/tnsm-ecmp-avg-reward-spiffy.tex
	}
	\cbend
\vspace{-0.25cm}
\end{table}
...
\cbstart Interestingly, SPIFFY is unable to effectively protect constant bitrate traffic. \cbend
...
\cbstart Here, SPIFFY greatly outperforms all of the RL-based methods. \cbend
...
\cbstart SPIFFY continues to outperform the RL-based methods. \cbend
...
We see that the initial increase in traffic volume causes the steepest reduction in performance (due to the increased cost of incorrect action), though performance levels out as attack traffic increases.
\cbend
...
Measurements taken during each of these experiments indicated that the cost of computing any action is typically within \SIrange{80}{100}{\micro\second} per flow.
...
\cbstart As discussed in \cref{sec:systems-considerations}, we are able to judge 3 flows before this deadline: this difference is primarily accounted for by serialisation/communication delays and single-threaded processing in the Python language. \cbend
...
What we offer above many of the approaches we discuss in \cref{sec:related-work} are potentially more flexible deployments and low-overhead decision-making, without requiring active measurement or the network resources and capabilities that the most effective techniques rely upon.
...
\cbstart
Can an attack on the controller can impact the effectiveness of our approach?
...
In a traditional network, only the agents act as controllers, but since agents periodically request per-flow data (rather than continuously receiving it) no amount of flows generates more requests or messages to the agent.
More work is generated, but we discuss how to handle this safely above.
Accordingly, the agent can never be stalled by request volume: the only communication with elsewhere in the network (load measurements) comes from trusted nodes, is highly periodic, and has constant size.
...
\cbstart
A limitation of SDN-capable hardware is that OpenFlow table entries occupy \SI{6}{$\! \times$} as much space as traditional routing rules---the cost and power demands of TCAM mean that commercial switches only have space for \SIrange{2}{20}{\kilo} rules \cite{DBLP:journals/comsur/NguyenSBT16}.
Our approach consumes an amount of rules equal to the number of active flows (i.e., the host density), and by the end of an experiment a switch may have accrued around ?? rules.
While we use a default action as a fallback to maintain connectivity, eviction of high-value decisions (those which filter the highest-bandwidth attackers) poses a significant risk.
Given that most flows are small (with the majority of bytes coming from a few large ``heavy-hitters'') \cite{DBLP:journals/ccr/PanBPS03}, it may suffice to only apply RL-based analysis to larger flows.
...
Given the high churn and prevalence of mice flows, eviction here is most likely to affect flows which no longer exist.
In both cases, extra rules can be made available by upgrading rules which completely filter a flow into an upstream blackholing request (as used in collaborative DDoS approaches \cite{DBLP:conf/acsac/RamanathanMYZ18}), having the agent remove this rule once blackholing is active.
\cbend
...
\cbstart Their assumptions about traffic response to such bandwidth expansion do not hold for constant bitrate flows (e.g., VoIP) and may not extend to HTTP DASH flows, both of which make up a sizeable proportion of network traffic. \cbend
