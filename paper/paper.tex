...
We define this to be $C_X = \min(\mathit{up}, \mathit{down})/\max(\mathit{up}, \mathit{down})$, where a value close to 0 indicates strong asymmetry.
...
Each is tile-coded with 4 tilings and 6 tiles per dimension, using the windows described in \cref{tab:codings}.
...
On the other hand, treating each individually allows us to achieve a quick understanding of the importance of each while seeing adequate performance.

\Cref{fig:udp-feature-plots} shows the effectiveness of each for UDP (resp.\ \cref{fig:udp-feature-plots} for TCP), using a topology with $n=2$ hosts per egress point.
...
Additional testing shows that the learned policies may be effectively combined (by summing action values), and are particularly effective for TCP; these results are omitted to preserve space.
...
It is anticipated that this difference might prove problematic for heterogeneous traffic mixtures one way of solving this may be to investigate the effect of replacing $U_s$ with e.g., $0.99 U_s$ in \cref{eqn:reward-but-better} to introduce a small band of negative reinforcement without actually removing the load restriction in the environment.
...
?? How to organise? by traffic type, or by model?
