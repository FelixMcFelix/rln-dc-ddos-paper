...
\Cref{fig:udp-feature-plots} shows the effectiveness of each feature for UDP (resp.\ \cref{fig:udp-feature-plots} for TCP), using a topology with $n=2$ hosts per egress point.
...
We now examine the performance of our two new models as compared against existing RL work under different traffic behaviour, for varying $n$.
...
\begin{figure}
	...
	\caption{
		Online performance for $n=2$ hosts per egress point.
		\label{fig:udp-2}
	}
\end{figure}
\begin{figure}
...
Yeah, Marl ++ is good. So what. \Crefrange{fig:udp-2}{fig:udp-16}
...
Yeah, Marl ++ is still good. So what. \Crefrange{fig:tcp-2}{fig:tcp-16}

\subsection{Banded rewards and negative reinforcement}

?? Generally? Not good.
?? While uncapped led to a per-feature improvement for UDP, Uncapped worse in all cases when all features included.

\subsection{Single-agent performance}

?? Curiously, MARL suffers but SPF actually become usable.
...
?? Most importantly this still trails behind existing (exact) DDoS flow detection mechanisms
