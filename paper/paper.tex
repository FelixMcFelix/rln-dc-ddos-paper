...
This formulation of interaction with a system is best described as a \emph{Markov decision problem} (MDP).
...
To cope with a continuous state- and/or action-space, one valuable technique is to employ \emph{tile-coding} \cite[pp.\ \numrange{217}{221}]{RL2E}---this converts our state from $\mathbb{R}^d \times \mathcal{A}$ into a sparse boolean feature vector $\operatorname{\mathbf{x}}(s, a)$ by subdividing it into a number of overlapping $(d+|\mathcal{A}|)$-dimensional grids with an optional bias component.
...
Given a weight vector $\wvec{0}=\bm{0}$ and learning rate $\alpha$, we may approximate an action $a$'s value in any state $s$ by $\operatorname{q}(s, a) \approx \acval{s}{a}{\wvec{}}$:
\begin{subequations}
	\begin{gather}
	...
	\label{eqn:sg-sarsa}
\end{subequations}
By computing the approximate value of every action available in the current state, actions with maximal value can be chosen each time (the \emph{greedy} policy), we might take a random action with probability $\epsilon$ to encourage early exploration (the \emph{$\epsilon$-greedy policy}), or we might some use some other mechanism.
...
Observing that $\nabla{\acval{s}{a}{\wvec{}}} = \operatorname{\mathbf{x}}(s, a)$, we may gain further optimisations by considering that a tile-coded feature vector is a boolean vector of constant Hamming weight (and so is amenable to representation as an array of indices).
Moreover, we need not store vectors of action values for tiles which have not yet been visited.
...
\caption{
	...
	Unfilled nodes are considered to be internal.
...
The original introduction of this approach to direct-control reinforcement learning as introduced by \textcite{DBLP:journals/eaai/MalialisK15} fails to consider key cases: the accurate simulation of TCP-like behaviour (and its effects on collateral damage), disjoint ranges of traffic distribution (i.e., the presence of benign heavy-hitters), and assumes a rigid ISP-like topology.
...
The .
In practice, well-established network topologies such as the \emph{fat-tree topology} fan out internally (to provide efficient replication and scaling of resources) while having comparatively few egress points.
It is this latter condition which I feel is most worth examining: the case where the host/agent ratio is heavily skewed in favour of hosts, and actions as they have been defined are forced to become less granular.

The base `online' experiment I replicate configures the network topology using $k=5$ teams, $l=3$ intermediate nodes per team, $m=2$ agents per intermediate node, and $n \in \{2, 7, 11, 14\}$ hosts per learner.
I set $\gamma=0$ (leading to opportunistic behaviour), $\alpha=0.05$ and linearly annealing $\epsilon=0.2 \rightarrow 0$ by $t=3000$.
Benign and malicious hosts uploaded between \SIrange{0}{1}{\mega\bit\per\second} and \SIrange{2.5}{6}{\mega\bit\per\second} respectively, and hosts are redrawn at each episode's start with $\operatorname{P}(\mathit{malicious})=0.4$.
$U_s$ was fixed at $klmn+2\si{\mega\bit\per\second}$.
The performance of each choice of $n$ was averaged over \num{10} episodes of length \num{10000} timesteps (setting $\wvec{}=\bm{0}$ between episodes).
...
\Cref{fig:marl-granularity} shows that allocating more hosts to each learner in the recreated experiment leads to a decrease in the amount of benign traffic observed at the server.
...
Notably, modelling the environment in Mininet makes evaluation rather slow as any interaction, coordination and communication must take place in real-time.
While I'm sure this enhances the quality of model learned due to the explicit inclusion of more accurate network behaviour, a more abstract simulator based on observed physical properties could provide data at less computational cost.

\subsection{TCP Multiplexing and Fallback Behaviour}
While Tcpreplay was useful in recreating the original work, flow behaviour under packet loss is closer to that of UDP when using it to replay traffic.
To validate the operation of the pdrop OpenFlow action and to investigate the effects of collateral damage upon benign TCP flows, I conducted an iperf3 test using mininet.
...
\Cref{fig:mplex-tcp} shows that benign TCP flows are severely harmed by \emph{any} packet drop, so collateral damage to these hosts is significant.
