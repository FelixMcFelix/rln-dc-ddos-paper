...
\hypersetup{
	...
	urlcolor=black,
	pdftitle={\mytitle{}},
%	pdfauthor={Kyle A. Simpson, Dimitrios P. Pezaros, Simon Rogers}
...
\begin{figure}
	...
	\caption{
		...
		SPF, remarkably, displays performance on par with Marl++ for this more difficult topology in spite of underperforming when $n=2$.
		Both of the new models take longer to train, but achieve better peak and average performance than Marl.
...
\begin{figure}
	...
	\caption{
		...
		SPF is slightly worse than Marl and Marl++ at low $n$, but achieves considerably better median performance and lower variance than the other models for higher $n$.
		\label{fig:tcp-box}
	}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{../plots/tcp-16-single}
	
	\caption{
		Online performance for $n=16$ hosts per egress point when benign traffic is TCP-like.
		At this level of host density, SPF reaches its peak performance sooner and is considerably more consistent throughout.
		Marl++ takes longer to reach its most effective policy, we achieve visibly better peak performance.
...
Where TCP-like traffic is concerned in both models, we see that this strategy almost always leads to worse policies.
...
\subsection{Per-feature pretraining}
...
We reduce the timestep length to \SI{10}{\milli\second} to train each agent in less wall-clock time, at the cost of noisier data (and reduced learning rate).
The results show no real change for Marl++, but a considerable gain for SPF against UDP (and performance drop against TCP).
It is likely that, as did the different protocols in \cref{fig:udp-feature-plots,fig:tcp-cap-feature-plots}, Marl++ and SPF have different optimal feature sets for different traffic behaviours---many of which have been excluded.
...
Moreover, our decision making processes are entirely agnostic with regards to the protocol or content of inbound traffic, offering future-proofing against the introduction of new transports.
...
\begin{figure}
	...
	\caption{
		Mean policy for Mean IAT when $n=16$.
		The column at any row is the action most likely to be chosen: this isn't actually a policy because it's mean'd.
		Repeated values originate from the bias tile, which is always active.
		?? Shows... what?
...
We may see this in \cref{fig:intern-16-tcp-iat}... ?? ELABORATE
...
We believe that it may be possible to implement similar behaviour on plain switches through application of \emph{programmable data planes} \cite{DBLP:conf/ancs/JouetP17}.
...
Our evaluation shows that our new agent designs advance the state-of-the-art in RL-based DDoS prevention, with SPF showing the most promise for future evaluation.
