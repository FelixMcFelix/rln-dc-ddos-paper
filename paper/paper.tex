...
\section{DDoS Mitigation with Per-flow Reinforcement Learning}
...
Our own analysis of CAIDA datasets \cite{caida-2018-passive} shows that congestion-aware traffic makes up at least \SIrange{73}{82}{\percent} of packets, corresponding to \SIrange{77}{84}{\percent} of data transferred by volume \footnote{\url{https://github.com/FelixMcFelix/caida-stats}}.
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{../plots/ftprep-cap-box}
	\vspace{-1.2cm}
	\caption{
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{../plots/ftprep-tcp-laf-cap-box}
	\vspace{-1.2cm}
	\caption{
...
\Cref{fig:tcp-laf-feature-plots} demonstrates how feature accuracy varies when tiled alongside \emph{last action}, with similar trends being observed when the same techniques is applied with UDP traffic (omitted).
...
\subsection{UDP (VoIP) Traffic Model}
We model 
?? Describe opus traffic model...\footnote{\url{https://github.com/FelixMcFelix/opus-voip-traffic}}.
?? Based on Discord\footnote{\url{https://discord.gg}}
?? Highly periodic, CBR
...
?? We use the traffic models introduced in \cref{sec:a-new-normal}.

?? We evaluate in multi-agent mode, and in single-agent mode (0-cost perfect information-sharing).
...
We compare our work most naturally against MARL \cite{DBLP:journals/eaai/MalialisK15}, the state-of-the-art in RL-based DDoS prevention.

The algorithm parameters were set at $\alpha=0.05$, having linearly annealed $\epsilon=0.2 \rightarrow 0$ by $t=3000$ in the case of the \emph{Instant} model.

?? Calculated expected malicious, keep with its other half.
...
??When UDP, \SIrange{2.5}{6}{\mega\bit\per\second}.
...
?? $U_s$ was fixed at $N_{\mathit{hosts}}+2$ \si{\mega\bit\per\second}
...
\begin{figure}
	...
	\caption{
		...
		Red nodes are considered to be external, and each blue node hosts an agent.
...
?? $n \in \{6, 12, 24, 48\}$ hosts per learner (keeping $N_{\mathit{hosts}}$ identical to each tier of the single-host topology)
?? Set $U_{s_0} = U_{s_1} = U_s / 2$

\section{Results}
\label{sec:the-results-of-doing-so}

We now examine the performance of our two new models (\emph{Instant}, \emph{Guarded}) as compared against existing RL work (\emph{Marl}) under different traffic behaviour and topologies, varying the host-to-learner ratio $n$.
...
\begin{figure}
	\centering
	\includegraphics[width=0.95\linewidth]{../plots/tnsm-tcp-box-separate}
...
\begin{IEEEbiography}
	...
	received the PhD from University of Bristol in 2006. Now he is a Senior Lecturer in the School of Computing Science at the University of Glasgow. Before starting a permanent  post in Glasgow he did a couple of Post Doctoral positions (under Mark Girolami).
	
	Dr.\ Simon Rogers’s research involves the development of Machine Learning and Statistical techniques to help with the analysis of complex datasets, particularly within the field of Metabolomics but also other fields, Human-Computer Interaction and Information Retreival.
\end{IEEEbiography}
