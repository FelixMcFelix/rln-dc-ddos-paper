...
\hypersetup{
	...
	urlcolor=black,
	pdftitle={\mytitle{}},
%	pdfauthor={Kyle A. Simpson, Dimitrios P. Pezaros, Simon D. Rogers}
...
Action values for any state are then obtained by summation of the weight vectors from all activated tiles---taking $|\mathcal{A}|(n_{\mathit{tilings}})$ floating point additions per decision.
...
This means that we need only perform $n_{\mathit{tilings}} + 2$ floating point additions and \num{2} multiplications per model update:
\begin{equation}
...
What we offer above the alternate approaches we discussed in \cref{sec:related-work} are potentially more flexible deployments and low-overhead decision-making, without requiring active measurement or the network resources and capabilities that the most effective techniques rely upon.
...
We still offer advantages above existing works, such as potentially more flexible deployment and automatically learned low-overhead decision-making, without requiring many of the same network resources or capabilities that they rely upon.
