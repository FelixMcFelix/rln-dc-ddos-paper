...
As exploration still occurs for each action, this approach reduces $\epsilon$ multiple steps for each $t$.
In turn, we increase the annealing window for $\epsilon$ to \num{8000} action choices to preserve exploration over time, while accounting for the greater volume of decisions being made.
...
While it is not stated whether this behaviour applies to amplification attacks, we expect this to remain the case; the upload rate of any amplifier will be fixed at its upper bound given that upstream links of bots are saturated.
...
Our approach is to model how an agent treats each flow using a simple finite state automaton: we restrict $p \in \left\{ 0.00, 0.05, 0.25, 0.50, 1.0 \right\}$, reducing the action set to \emph{maintain}, \emph{increase}, or \emph{decrease} $p$ in single steps.
...
This model is significantly different from the perspective of RL parameter tuning: each agent now requires the capability to plan ahead, since they may no longer choose $p$ directly from any state.
