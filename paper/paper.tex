...
We examine how the paradigm has been applied to the task of DDoS prevention to date, and present an analysis of suspected edge cases to uncover existing weaknesses in present approaches.
...
We advance the state-of-the-art of DDoS mitigation using RL by introducing two agent classes designed to act on a per-flow basis, supported by an in-depth investigation of feature suitability and empirical evaluation.
...
While \emph{machine learning} (ML) approaches seem like a sensible fit for this problem, in \citeyear{DBLP:conf/sp/SommerP10} \citeauthor{DBLP:conf/sp/SommerP10} identified the `failure to launch' of ML-based anomaly detection systems---to quite a large extent, this remains the case today \cite{DBLP:conf/sp/SommerP10}.
...
%	\item A DDoS mitigation system based on direct-control reinforcement learning, designed for deployment in real-world software-defined networks (\cref{sec:environment-and-rl-algorithm,sec:acting-on-individual-sources}).
	...
	\item Two source-level granularity approaches to RL-driven DDoS prevention, improving upon past aggregate-based models (\cref{sec:acting-on-individual-sources}), alongside an empirical evaluation (\cref{sec:the-results-of-doing-so}).
\end{itemize}
...
This means that we need only perform $n_{\mathit{tilings}}$ floating point additions per model update:
\begin{equation}
...
Furthermore, networks of this kind more naturally enable the future use of \emph{network function virtualisation}, a technology which could allows relocation and easy installation of learners.
...
In essence, this means that each agent is in control of pushback \cite{DBLP:journals/ccr/MahajanBFIPS02a}, and so exhibit a risk of introducing collateral damage into the network.
These choices primarily ensure that this rate reduction imposed on a source IP may never be performance.
...
We contribute a network model built around live testing of TCP traffic in an SDN-enabled environment, adaptable to arbitrary topologies, with an explicit focus on preserving the real-time dynamics of TCP traffic.
...
\end{itemize}
We offer these to model two extremes: long-life host-to-server transfers (a characteristic we might e	xpect from e.g., streaming video), and common HTTP traffic.
...
This aligns with our goal of quick online learning, and potentially faster adaptation to aggregate changes in traffic without the introduction of dedicated tensor processing hardware to networks.
...
Recounting \cref{sec:reinforcement-learning}, the choice of $\gamma$ affects which fraction of the next current state-action pair's value is added to $R_t$ when updating the last state-action pair's value.
...
\begin{figure}
	\includegraphics[width=0.8\linewidth]{../plots/ftprep-good-2}
	\caption{
...
It is crucial to make this region as small as possible, hence the somewhat arbitrary choice; we observe that agents are always able to learn how to keep traffic consistently below the logical bandwidth limit (as in the original work).
...
Additionally, we examine the performance effects of environmental characteristics and potential improvements: negative reinforcement, and the case where one agent makes all decisions.%, and pre-training on individual features.
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{../plots/udp-box}
	
	\caption{
		Online performance  when benign traffic is UDP-like.
		...
		SPF, remarkably, displays performance on par with Marl++ for this more difficult topology in spite of performing poorly when $n=2$.
		Both of the new models take longer to train, but achieve better peak and average performance than Marl.
		The longer tails of outliers typically indicate the longer training time the new models require.
		\label{fig:udp-16}
	}
\end{figure}
...
\Cref{fig:udp-16} shows the behaviour for all choices of $n$ at a high level---specifically, we see that Marl++ typically remains ahead of Marl, though this performance difference shrinks as $n$ increases.
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{../plots/tcp-box}
	
	\caption{
		Online performance for when benign traffic is TCP-like.
		Marl and Marl++ begin with essentially identical efficacy.
		Although Marl++ takes longer to reach its most effective policy, we achieve visibly better peak performance.
		SPF does manage to learn an improved policy compared to its starting state, but never manages to perform well.
		\label{fig:tcp-16}
	}
...
\Cref{fig:tcp-16} shows a trend in our results: for larger $n$, Marl++ requires around twice as long as Marl to learn its best-achieving policy, underperforming for this period, but achieves notably better peak and average performance.
...
Of the results presented, SPF's unpredictable (often worse) performance is unexpected, given its far smaller action space.
...
What we offer above the alternate approaches we discussed in \cref{sec:related-work} are potentially more flexible deployments and the ability to adapt by design to changing network traffic characteristics, without requiring the network resources or capabilities that the most effective techniques rely upon.
...
Where overheads are concerned, the state space sizes guarantee that a Marl policy remains under \SI{427}{\kibi\byte}, although in practice our sparse representation typically leads to far smaller policies---\SIrange{100}{120}{\kibi\byte} from our experiments.
...
We believe that it may be possible to implement similar behaviour through application of programmable data planes.
...
Gathering and transmission of load/flow statistics would be difficult to perform quite as often as we can in an emulated environment, without inadvertently affecting host traffic.
However, the measurements acquired in such a scenario are far more likely to be less noisy (by being drawn over longer periods of time), which could aid effective training.
The main bottlenecks are most likely in forwarding the load measurements from various aggregation points (which can be made more efficient by way of multicast) and in running some estimator to condition the reward function upon.
...
We expect that agents will be able to share policies for all fields except global state, which may help to offset the reduced rate of incoming experience.
Regardless, it will take longer to achieve enough experience to converge on a good policy.
...
Our evaluation shows that our new agent designs advance the state-of-the-art in RL-based DDoS prevention.

While this work still trails behind the performance of exact DDoS flow detection mechanisms, we hope it is clear that the reinforcement learning paradigm holds promise and can inspire further innovation.
Yet even now, we offer advantages above these works, such as online adaptation and potentially more flexible deployment, without requiring many of the same network resources or capabilities that they rely upon.
