...
These flaws---poor handling of TCP traffic and high host density at egress points---catalyse and direct an exploration of the design space of RL models.
We improve the state-of-the-art through two agent classes designed to act on a per-flow basis, supported by an in-depth investigation of feature suitability and empirical evaluation.
...
%	\item A DDoS mitigation system based on direct-control reinforcement learning, designed for deployment in real-world software-defined networks (\cref{sec:environment-and-rl-algorithm,sec:acting-on-individual-sources}).
	\item Important weaknesses and flaws in the past design and evaluation of similar techniques \cite{DBLP:journals/eaai/MalialisK15}---a deconstruction and empirical study on their formulation, flaws, and risk factors concerning different traffic distributions (\cref{sec:performance-in-an-emulated-environment}).
...
While $\bload{\cdot}$ is likely to be a good candidate for many deployments, we believe that internal ``canary flows'' (or other active measurement) will be effective.
...
Effective real-world deployment of RL-based defences cannot assume that switches in a network will support a custom version of OVS or other arbitrary software, introducing the question of whether agent training, execution and distribution may when using \emph{programmable data planes} \cite{DBLP:conf/ancs/JouetP17}.
...
We stress once again that this work still trails behind the performance of exact DDoS flow detection mechanisms, yet we hope it is clear that the reinforcement learning paradigm holds promise and can inspire further innovation.
