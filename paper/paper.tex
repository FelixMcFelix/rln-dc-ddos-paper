...
?? RESTRUCTURE
These authors posit that their application is made difficult due to significant operational differences from standard ML tasks.
?? (quoted: high cost of errors, lack of training data, semantic gap between results and operational interpretation, variability in input data, fundamental difficulties for conducting sound evaluation)
This is further aggravated by certain characteristics of network traffic; the diversity of network traffic across varying timescales \cite{DBLP:conf/sp/SommerP10} and significant burstiness \cite{DBLP:journals/ccr/LelandWTW95}.
...
%			\filldraw (0.625,-0.125) circle[radius=1.5pt,label=above:{$s$}];
	...
	\begin{subfigure}{0.4\linewidth}
		...
		\caption{Action Selection\label{fig:tilecode-select}}
	\end{subfigure}
...
\Cref{fig:tilecode-code} demonstrates the process for a 2-dimensional state space, and how the numbers of tilings and tiles per dimension control feature resolution and generalisation.
...
\Cref{fig:tilecode-select} extends our working example to show how the value of each action is computed, combining a global estimate ($T_{\mathit{bias}}$) with knowledge particular to each state.
...
\caption{
	...
	For illustrative purposes, we choose an unrealistically high $\alpha$ (typically, $\alpha=0.05$ or lower is a more reasonable choice).
