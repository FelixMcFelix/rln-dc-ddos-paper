...
Currently, we assume that switches support an extension to OpenFlow to enable remotely installable packet-drop rules, either by running a modified version of OVS on commodity hardware at these locations or through custom firmware for one of these particular egress switch.
Similar functionality could be employed by making use of OpenFlow's meter rules.
As far as footprints are concerned, although the state space sizes guarantee that a Marl policy remains under \SI{427}{\kibi\byte}, in practice our sparse representation typically leads to far smaller policies---\SIrange{100}{120}{\kibi\byte} from our experiments.
Beyond this, we require that egress switches are capable of co-hosting an agent (i.e., through \emph{network function virtualisation}, with the necessary statement.)
%?? Incorrect, pickling...
...
A real world deployment would be unable to gather and transmit load/flow statistics quite as often as we can in an emulated environment, without inadvertently affecting client traffic.
However, the measurements acquired in such a scenario are far more likely to be less noisy (by drawing state over longer periods of time), and so could greatly aid effective training.
We do not forsee this overly harming training.
...
For non-tree-structured topologies, we each a
