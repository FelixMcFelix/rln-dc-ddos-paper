...
%	\item Important weaknesses and flaws in the past design and evaluation of similar techniques \cite{DBLP:journals/eaai/MalialisK15}. We offer a deconstruction and empirical study on their formulation, flaws, and risk factors concerning different traffic distributions---in addition, we demonstrate modifications enabling deployment in real-world software-defined networks (\cref{sec:performance-in-an-emulated-environment}).
	...
	\item An investigation into suitable features for an automatic DDoS mitigation system (\cref{sec:rethinking-the-state-space}).
%	\item Reactive simulations of HTTP and VoIP web-server traffic, designed to test system characteristics that packet trace playback fails to capture (\cref{sec:a-new-normal}).
	\item A comparison of the effectiveness of different RL algorithms in these agents (\cref{sec:the-results-of-doing-so}).
...
To make decisions and learn cheaply with low latency, we use \emph{semi-gradient Sarsa with tile coding} \ref{eqn:sg-sarsa} (\cref{sec:reinforcement-learning}), rather than make use of neural networks.
...
While classical RL methods likely have lower accuracy, there are reasons to favour them; faster decisions, lower energy usage, reduced model complexity (and training time), and the availability of compatible hardware.
...
Approaches like this have simpler decision boundaries, reducing the risk of overfitting and unexpected behaviour. 
Simpler methods of computing policies may also aid in interpreting anomalous action choices.
...
In particular, we assume that each agent operates as a \emph{virtual function} (VF) adjacent to a software-defined switch.
...
A weakness of their approach is that computing a route with enough spare bandwidth can be costly, and that low expansion factors in real network can require more ``rounds'' of testing. 
...
\emph{Athena} \cite{DBLP:conf/dsn/LeeKSPY17} is a generalised SDN framework for intrusion detection, but shows the use of a \emph{k-nearest neighbours} classifier to detect individual attack flows.
Although heavyweight (and proven to be effective compared with \textcite{DBLP:conf/lcn/BragaMP10}), their comparison against SPIFFY lacks the some evidence needed to understand how the system compares.
...
The approach uses upon BGP \emph{fraudulent route reverse poisoning} to preserve traffic to a target AS, but unlike SPIFFY the approach doesn't actually \emph{remove} the congestion.
Because of this, traditional flooding-based attacks aren't fully alleviated.

%?? Abuses of RL 
\fakepara{RL in Networks}
Application of RL towards the challenge of intrusion detection/prevention has seen comparatively little examination.
Past work uses the approach as a traditional classifier for anomaly detection \cite{shamshirband2014anomaly} and DDoS prevention \cite{DBLP:conf/mates/ServinK08}.
...
A past example which embodies a different use learns when it is best to \emph{communicate} and share knowledge between \emph{hidden Markov model} anomaly detectors \cite{DBLP:conf/paisi/XuSH07}.
...
Outside of anomaly/intrusion detection, there has been growing interest in the use of reinforcement learning in data-driven networking, such as for intra-AS route optimisation \cite{DBLP:conf/hotnets/ValadarskySST17} and for process allocation \cite{DBLP:conf/hotnets/MaoAMK16}.
