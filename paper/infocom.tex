...
%	pdfauthor={Kyle A. Simpson, Simon Rogers, Dimitrios P. Pezaros}
	pdfauthor={Anonymous Giraffe, Anonymous Ibex, Anonymous Badger}
}
...
Transfer of training data and execution can be expensive, hampering deployment.
In this paper, we show how to deploy such RL agents in a typical network at low cost through a novel architecture.
Our two agent designs make better use of flow information to advance the state-of-the-art in using RL for DDoS mitigation.
...
Our results show that flow features effective on bulk TCP and VoIP traffic, combined with our agent designs, protect \SI{8}{$\!\times$} more TCP traffic than no defence at high host density.
This is a \SI{2.5}{$\!\times$} improvement over RL's state-of-the-art in DDoS defence.

%(\SI{2.5}{$\!\times$} improvement over Marl, \SI{8}{$\!\times$} more traffic than no defence system)
\end{abstract}
...
%We hope it is clear that RL holds promise and can inspire further innovation.
