...
def write_table(out_file, data):
	...
	for traffic in traffics:
		for i, n in enumerate(ns):
			...
			to_underline = np.argmax(row[1:]) + 1

			for j, val in enumerate(row):
				uline_part = "{:.3f}" if j != to_underline else "\\uline{{{:.3f}}}"

				base = ("& {} " if j != to_bold else "& \\bfseries {} ").format(uline_part)
	...
	for t_i, t in enumerate(traffics):
		for n in ns:
			...
			row.append(get_average_reward(
				"../results/tnsm-ecmp-{}-{}-spiffy-separate.csv".format(
					n,
					t,
					)
				))
