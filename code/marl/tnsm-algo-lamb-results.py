...
def total_per_model(model):
	return len(discounts) * len(algos) * len(lambs) * len(host_ps) * len(traffic_types) * (1 if model == 0 else len(single_learners)) * len(maths) * len(topols)
...
if __name__ == "__main__":
	...
	file_name_part = "tnsm-algo-lamb-{}-{}-{}-{}-{}-{}-{}".format(algo, lamb, topol, host_p, traffic_prefix, model_prefix, indiv_prefix)
